...
export default function Scope({engine,patch,selectedId,onSelect,ready}:Props){
 ...
 const effectiveId=selected?.id;const signal=selected?.type==='output'?'Master output':selected?`${MODULE_SPECS[selected.type].name} · ${selected.id}`:'Master output';
 useEffect(()=>{
